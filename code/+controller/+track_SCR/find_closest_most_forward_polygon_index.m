function polygon_index = find_closest_most_forward_polygon_index(position, track_upscaled_polygons)
...
for j = 1:length(track_upscaled_polygons)
    ...
    distance_to_polygon = max(track_upscaled_polygons(j).A * position - track_upscaled_polygons(j).b);
