...
for vel_index = [1 2 3] % [v v_long v_lat]
    %% Plot track
    if vel_index == 1
        figure_handle_number = 1020;
    elseif vel_index == 2
        figure_handle_number = 1021;
    else
        figure_handle_number = 1022;
    end
    f = figure(figure_handle_number);
    clf(f)
    plots.Race(figure_handle_number).plot_track(cfg.scn.track);
    hold on
    if vel_index == 1
        set(figure_handle_number, 'Name', "Vehicle 1's Velocity v on Track");
    elseif vel_index == 2
        set(figure_handle_number, 'Name', "Vehicle 1's Longitudinal Velocity v_{long} on Track");
    else
        set(figure_handle_number, 'Name', "Vehicle 1's Lateral Velocity v_{lat} on Track");
    end
