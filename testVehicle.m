[a_forward_max, a_backward_max, a_lateral_max] = get_a_max([cfg.tempPath 'singleTrackAMax.mat']);

% debug with calculate_a_max(true)
% production usage with get_a_max(<filename>)
function [a_forward_max, a_backward_max, a_lateral_max] = get_a_max(filename)
    if ~isfile(filename)
        a_max = calculate_a_max();
        a_forward_max = a_max(1, :);
        a_backward_max = a_max(2, :);
        a_lateral_max = a_max(3, :);
        save(filename, 'a_forward_max', 'a_backward_max', 'a_lateral_max');
    else
...
switch mode
    case mode_long_forward
        ...
        v_measure = 0:0.01:v_max;
        
        % find changing velocity indices only 
        i_velocity_changing = 1:(find(round(x(:, 3), 4) == v_max, 1));
        
        a_forward_max = interp1(x(i_velocity_changing, 3), a_measured(i_velocity_changing, 1), v_measure);
        
        %% longitudinal backwards (recursive call)
        a_backward_max = calculate_a_max(shallPlot, 11, v_max, v_measure);
        ...
        a_lateral_max = interp1(vs, a_lateral_max_all(:, i_delta)', v_measure);
        
        output_ = [a_forward_max; a_backward_max; a_lateral_max];
        
        if shallPlot
            figure('Name', 'a_max over v_x');
            plot(v_measure, output_)
    ...
    case mode_long_backward
        ...
        output_ = interp1(x(i_positive_velocity, 3), a_measured(i_positive_velocity, 1), v_measure);
    case mode_lat
